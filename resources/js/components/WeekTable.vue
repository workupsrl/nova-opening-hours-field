        ...
        <tbody>
            <tr v-for="day in week" :key="day.day" class="group">
                ...
                <table-column v-if="editable" class="text-right">
                    <add-button @click.prevent="$emit('addInterval', 'week', day.day)"><span class="px-1">+</span></add-button>
                    <span v-if="Object.values(day.intervals).length" class="ml-2">
                        <remove-button @click.prevent="$emit('removeAllIntervals', 'week', day.day)"><span class="px-1">-</span></remove-button>
                    </span>
                </table-column>
            </tr>
        </tbody>
