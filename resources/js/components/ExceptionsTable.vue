        ...
        <thead class="bg-gray-50 dark:bg-gray-800">
            <tr>
                ...
                <table-header v-if="editable" class="text-right">
                    <add-button @click.prevent="$emit('addException')"><span class="px-1">+</span></add-button>
                </table-header>
            </tr>
        </thead>
        <tbody>
            <tr v-for="exception in exceptions" class="group">
                ...
                <table-column v-if="editable" class="text-right">
                    <add-button @click.prevent="$emit('addInterval', 'exceptions', exception.date)"><span class="px-1">+</span></add-button>
                    &nbsp;
                    <remove-button @click.prevent="$emit('removeException', exception.date)"><span class="px-1">-</span></remove-button>
                </table-column>
            </tr>
        </tbody>
    </table>
